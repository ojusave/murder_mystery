...
export async function POST() {
  try {
    ...
    console.log('FAQ table exists:', tableExists[0].exists)
    ...
    if (!tableExists[0].exists) {
      ...
      // Create indexes
      await prisma.$executeRaw`CREATE INDEX "faqs_is_active_idx" ON "faqs"("is_active");`
      await prisma.$executeRaw`CREATE INDEX "faqs_order_idx" ON "faqs"("order");`
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'FAQ database fix completed successfully!',
      faqCount: existingCount
    })
    
  } catch (error) {
    console.error('FAQ database fix failed:', error)
    return NextResponse.json({ 
      success: false, 
